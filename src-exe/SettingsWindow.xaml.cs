...
namespace RustFix
{
    public partial class SettingsWindow : Window
    {
        ...
        private void SetPauseDays(int days)
        {
                ...
                using (var key = Registry.LocalMachine.CreateSubKey(@"SOFTWARE\Microsoft\WindowsUpdate\UX\Settings"))
                {
                    if (key != null)
                    {
                        if (days <= 0)
                        {
                            ...
                            MessageBox.Show("Пауза обновлений снята.", AppTitle, MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                        else
                        {
                            ...
                            MessageBox.Show($"Обновления успешно приостановлены на {days} дней (до {DateTime.Now.AddDays(days):dd.MM.yyyy}).", AppTitle, MessageBoxButton.OK, MessageBoxImage.Information);
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show("Ошибка изменения параметров реестра: " + ex.Message, AppTitle);
            }
        }
        ...
        private void BtnApplyPolicies_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("Блокировки успешно применены в реестре и службах Windows.", AppTitle, MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Ошибка: " + ex.Message, AppTitle);
            }
        }
        ...
        private void BtnRestoreDefaults_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("Штатный режим обновлений Windows восстановлен.", AppTitle, MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Ошибка восстановления: " + ex.Message, AppTitle);
            }
        }
